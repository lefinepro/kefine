import { O as writable } from './exports-CoBtNwQi.js';

//#region src/lib/constants/kefine-locale-en.ts
var KEFINE_TEXT_EN = {
	meta: { locale: "en" },
	brand: { name: "Lefine" },
	topbar: {
	...
	status: {
		statusPollingMessage: (solver, queued) => `Solver ${solver} is ${queued ? "accepting" : "executing"} the task`,
		createSending: "Sending task...",
		queued: (solver) => `Task queued. Assigned solver: ${solver}`,
...
//#endregion
//#region src/lib/constants/kefine-locale-hy.ts
var KEFINE_TEXT_HY = {
	...KEFINE_TEXT_EN,
	meta: { locale: "hy" },
	topbar: {
...
//#endregion
//#region src/lib/constants/kefine-locale-ru.ts
var KEFINE_TEXT_RU = {
	meta: { locale: "ru" },
	brand: { name: "Lefine" },
	topbar: {
	...
	create: {
		...
		addFile: "+ файл",
		addPrice: "+ цена",
		fileCount: (count) => `${count} файл${count === 1 ? "" : count < 5 ? "а" : "ов"}`,
	...
	status: {
		statusPollingMessage: (solver, queued) => `Решатель ${solver} сейчас ${queued ? "принимает" : "исполняет"} задачу`,
		createSending: "Отправка задачи...",
		queued: (solver) => `Задача в очереди. Назначенный решатель: ${solver}`,
...
//#endregion
//#region src/lib/constants/kefine-locale.ts
var LOCALE_STORAGE_KEY = "kefine-locale";
var KEFINE_LOCALE_TEXT = {
	en: KEFINE_TEXT_EN,
	hy: KEFINE_TEXT_HY,
	ru: KEFINE_TEXT_RU
};
function readLocaleFromStorage() {
	...
	if (savedLocale === "ru" || savedLocale === "en" || savedLocale === "hy") return savedLocale;
}
var kefineLocale = writable(readLocaleFromStorage() ?? "en");
function setKefineLocale(locale) {
...
export { getLocaleText as g, kefineLocale as k, setKefineLocale as s };
//# sourceMappingURL=kefine-locale-Dc0dhHaI.js.map
