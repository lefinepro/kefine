export const KEFINE_TEXT_RU = {
  ...
  create: {
    ...
    pinnedServicesSubtitle: 'Начните с настроенных точек входа сервиса перед созданием произвольного lepo.',
    pathsTitle: 'Пути lepo',
  ...
  weatherWidget: {
    ...
    geolocationPrompt: 'Разрешите доступ к геолокации, чтобы показать прогноз рядом с вами.',
    ...
    loadingNamed: (location: string) => `Загружаем прогноз для ${location}...`,
    locationNotFound: (location: string) => `Не удалось найти ${location}.`,
    forecastUnavailable: 'Прогноз погоды недоступен.',
    typeCityHint: 'Введите город, например Weather Gomel.',
    unitLabel: 'Единицы температуры',
    celsius: 'Цельсий',
    fahrenheit: 'Фаренгейт',
    metricsLabel: 'Детали погоды',
  ...
  status: {
    statusPollingMessage: (solver: string, queued: boolean) =>
      `Решатель ${solver} сейчас ${queued ? 'принимает' : 'исполняет'} lepo`,
    createSending: 'Пересылка документа...',
    queued: (solver: string) => `Lepo в очереди. Назначенный решатель: ${solver}`,
    notCompleted: 'Решатель ещё не вернул статус выполнения. Обновите статус lepo вручную.',
  ...
  labels: {
    ...
    searchTasks: 'Поиск lepos',
    noTasks: 'Lepos не найдены',
    noTasksDescription: 'Создайте хотя бы один lepo или измените строку поиска.',
    openPaymentInvoice: 'Открыть счёт',
    openOrderLink: 'Открыть заказ',
    loadMoreHint: 'Прокручивайте список, чтобы загрузить больше lepos',
    executingNow: 'Ваш lepo исполняется',
    orderStatusUnavailable: 'Ожидание обновления статуса lepo...',
    ...
    taskQueue: 'Очередь lepos',
    taskTree: 'Дерево lepo',
    taskFeed: 'Лента lepo',
  ...
  buttons: {
    apply: 'Применить',
    createNewTask: 'Создать новый lepo',
    openAllTasks: 'Открыть все lepos',
    createTask: 'Создать lepo',
    stopTask: 'Остановить lepo',
    socialLogin: 'Соцлогин',
    passkeyLogin: 'Passkey вход',
    actor: 'Workspace',
    editTaskFeed: 'Редактировать ленту lepo',
  ...
  auth: {
    ...
    signedIn: 'Вы вошли в систему. Можете отправить lepo.',
    reown: 'Reown',
    passkey: 'Passkey',
  ...
  profile: {
    ...
    signOut: 'Выйти',
    authDrawerSubtitle: 'Быстрый доступ к workspace и последним lepos.',
    openPublicProfile: 'Открыть публичный workspace',
    latestTasks: 'Последние lepos',
    projects: 'Проекты',
    lepos: 'Lepos',
    closedTasks: 'Закрытые выполненные lepos',
    publicTask: 'Сделать lepo публичным',
    ...
    publicTasks: 'Публичные lepos',
    noPublicTasks: 'Публичных lepos пока нет',
    noRecentTasks: 'Последних lepos пока нет',
    noOwnerTasks: 'Завершённых lepos пока нет',
    openTask: 'Открыть lepo',
    buyView: 'Купить просмотр',
    buyWatch: 'Следить за статусом',
    buyJoin: 'Присоединиться к lepo',
    ownerTask: 'Ваш lepo',
