...
export function pickOpenMeteoGeocodeResult(query: string, response: OpenMeteoGeocodeResponse): WeatherResolvedLocation | null {
  ...
  for (const item of response.results) {
    if (!Number.isFinite(item.latitude) || !Number.isFinite(item.longitude) || !item.name?.trim()) {
      continue;
    }
    ...
    // Prefer results with country info over bare names
    if (!best.country && item.country) {
      best = item;
      continue;
    }

    // Prefer the first valid result as initial tie-breaker
    if (!best.name) {
      best = item;
    }
  }

  if (!best || best.latitude === undefined || best.longitude === undefined || !best.name) {
    return null;
  }

  return {
    name: best.name.trim(),
    country: best.country?.trim() || undefined,
    countryCode: best.country_code?.trim() || undefined,
