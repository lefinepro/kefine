...
const modes = [
  { name: 'form', selector: '[data-body-mode="form"]' },
  { name: 'json', selector: '[data-body-mode="json"]' }
];
const viewports = [
...
for (const mode of modes) {
  for (const { tag, width, height } of viewports) {
    ...
    const fileUrl = 'file://' + resolve(here, 'solver-detail-testing-demo.html');
    ...
    for (const theme of ['light', 'dark']) {
      ...
      const out = resolve(outDir, `solver-testing-r19-${mode.name}-${tag}-${theme}.png`);
